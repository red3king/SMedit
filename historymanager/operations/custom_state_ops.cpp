...
OpCustomStateClassReload::OpCustomStateClassReload(CustomStateClass* customstateclass, CustomStateClass to_copy) : CustomStateClassChgOperation(customstateclass)
{
    this->to_copy = to_copy;
}


OpCustomStateClassReload* OpCustomStateClassReload::clone()
{
    return new OpCustomStateClassReload(*this);
}


void OpCustomStateClassReload::execute_impl(CustomStateClass* customstateclass)
{
    customstateclass->transition_defs = to_copy.transition_defs;
    customstateclass->configuration = to_copy.configuration;
    customstateclass->name = to_copy.name;
}


bool OpCustomStateClassReload::may_collapse_impl(Operation& other)
{
    auto op = (OpCustomStateClassReload&) other;
    return customstateclass_id == op.customstateclass_id;
}


void OpCustomStateClassReload::collapse(Operation& other)
{
    auto op = (OpCustomStateClassReload&) other;
    to_copy = op.to_copy;
}
