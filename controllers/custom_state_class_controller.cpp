...
int count_and_delete(vector<Machine*>& machines, unsigned int selected_csc_id, bool delete)
{
...
{
    int csc_count = count_and_delete(history_manager->current_project.machines, selected_csc_id, false);
    ...
    count_and_delete(history_manager->current_project.machine, selected_csc_id, true);

    auto op = OpCustStateDelete(selected_csc);
    history_manager->submit_operation(op);
}
